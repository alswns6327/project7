...
//		      System.out.println(obj.toString());
	...
	public void phoneNumCheck(HttpServletResponse response ,HttpServletRequest request, Model model) {
	    ...
	    try {
	      ...
	      if(test.substring(test.length()-2,test.length()-1)!="1") {
	    	  response.getWriter().write("1");
	      }else {
	    	  System.out.println("randNum 2 "+randNum.toString());
	    	  response.getWriter().write(randNum.toString());
	...
	public String pay(HttpServletRequest request, Model model) {
		...
		try {
			URL url = new URL("https://kapi.kakao.com/v1/payment/ready");
				...
				connection.setRequestProperty("Authorization", "KakaoAK ff80627b31983ffc255d7716b99f3824");
				...
				if(gradeCheck==0) {
					...
					String parameter = "cid=TC0ONETIME&partner_order_id=partner_order_id&partner_user_id="+id+"&item_name="+grade+"&quantity=1&total_amount="+price+"&vat_amount=0&tax_free_amount=0&approval_url=http://localhost:9095/project/homePage&fail_url=http://localhost:9095/project/homePage&cancel_url=http://localhost:9095/project/homePage";
